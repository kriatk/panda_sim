...
class PredictiveCoding:
    ...
    def __init__(self):
        ...
        # Subscribers
        self.camera     = None
        self.camera_sub = rospy.Subscriber("/camera1/image_raw", Image, self.camera_sub_callback)
        ...
        self.new_model_path = os.getcwd() + '/models/' + self.model_name
#        self.trained_w_path = exp_dir + self.model_name    # exp_dir computed in specs.py 
        self.trained_w_path = os.getcwd() + '/models/' + self.model_name      
    ...
    def run_step(self):
        ...
        if self.camera is not None:
            t = self.camera.header.stamp.to_sec()*1000.0   # in milliseconds
            #reset sim for training
            if t > self.last_cam_time + 20*self.underSmpl:           # one ros time-step is 20 ms
                ...
                if self.model is not None:
                    ...
                    if self.running_step > self.nt:
                        ...
                        # K= [190.68123344056778, 0.0, 160.5] 
                        #    [0.0, 190.68123344056778, 120.5] 
                        #    [0.0, 0.0, 1.0]


#                        pos_3d_msg = [[1.562-p[0]/156.274, -0.14-p[1]/152.691, 0.964+p[0]-p[0]] for p in targ_pos]
                        pos_3d_msg = [[-0.5+p[0]/120.5, 1-p[1]/160.5, p[0]-p[0]] for p in targ_pos]
...
if __name__ == "__main__":
    rospy.init_node('PredictiveCoding', anonymous=True)
    m = PredictiveCoding()
